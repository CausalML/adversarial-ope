...
class RobustFQILearner(AbstractLearner):
    ...
    def train(self, dataset, pi_e_name, evaluate_pv_kwargs=None, batch_size=1024,
              max_num_iterations=100, max_no_improve=10, num_restart=3,
              beta_reg_alpha=1e-5, q_reg_alpha=1e-5, min_q_change=1e-5,
              val_frac=0.1, add_q_multiplier_correction=False, verbose=False, device=None):
        ...
        dl_beta = dl
        dl_q = dl
        dl_all = dataset.get_batch_loader(batch_size)
        ...
        for restart_i in range(num_restart):
            try:
                ...
                next_min_err, next_best_state = self._try_single_train(
                    dl_beta=dl_beta, dl_q=dl_q, dl_val=dl_val, dl_all=dl_all,
                    add_q_multiplier_correction=add_q_multiplier_correction,
                    pi_e_name=pi_e_name, evaluate_pv_kwargs=evaluate_pv_kwargs,
                    batch_size=batch_size, max_num_iterations=max_num_iterations,
                    max_no_improve=max_no_improve, num_restart=num_restart,
        ...
        if add_q_multiplier_correction:
            if verbose:
                print('')
                print("adding final multiplier corection to Q from all train/dev data")
            self.model.add_q_multiplier_correction(
                dl_all,
                lmbda=self.adversarial_lambda,
                pi_e_name=pi_e_name,
                use_dual_cvar=self.use_dual_cvar,
    ...
    def _try_single_train(self, dl_beta, dl_q, dl_val, dl_all, pi_e_name,
                evaluate_pv_kwargs=None, add_q_multiplier_correction=False,
                batch_size=1024, max_num_iterations=100, max_no_improve=10,
                num_restart=3, beta_reg_alpha=1e-5, q_reg_alpha=1e-5,
                min_q_change=1e-5, verbose=False, device=None):
        min_err = float("inf")
        best_state = None
        num_no_improve = 0
        ...
        for iter_i in range(1, max_num_iterations+1):
            ...
            # if doing scalar update, do this now
            if (iter_i > 1) and add_q_multiplier_correction:
                self.model.add_q_multiplier_correction(
                    dl_q,
                    lmbda=self.adversarial_lambda,
                    pi_e_name=pi_e_name,
            ...
            q_err = self.compute_q_error(dl_val, pi_e_name)
            if verbose:
    ...
    def compute_q_l2_change(self, dl, pi_e_name, batch_scale=1000.0):
        ...
        for batch in dl:
            s = batch["s"]
            a = batch["a"]
            q_err = self.model.get_q(s, a) - self.prev_model.get_q(s, a)
            q_err_scaled = q_err * (1 - self.gamma)
            square_err = q_err_scaled ** 2
            batch_weight = len(batch["s"]) / batch_scale
            squared_err_sum += batch_weight * float(square_err.mean())
            batch_sum += batch_weight
        return (squared_err_sum / batch_sum) ** 0.5